...
def info_nce_force_emb_reg(energy_function_list, N_neg, force_emb_,force_emb, a_t, DEVICE = 'cuda'):
...
def info_nce_reg(energy_function_list, N_neg, emb_dic, idx_list, idx_c, idx_f, a_t, DEVICE = 'cuda'):
...
def info_nce(energy_function, N_neg, emb_dic, idx_list, idx_c, idx_f, a_t, DEVICE = 'cuda'):
    p_ = energy_function(torch.cat([ emb_dic(idx_c) , emb_dic(idx_f), a_t], axis=0))
    
    p_ = torch.clip(p_, -5, 5)
    p = torch.exp(-p_)
    
    idx_list = np.delete(idx_list , np.where(idx_list ==  int(idx_f)*np.ones_like(idx_list) ))
...
def hyper_loss(model_output, gt_sdf, gt_normals, ks, ki, kn, kg):
    ...
    sdf_constraint = torch.where(gt_sdf == 0, torch.abs(pred_sdf), torch.zeros_like(pred_sdf))   
    
    pred_sdf_c = torch.clip(pred_sdf, -0.3, 0.3).float()
    gt_sdf_c = torch.clip(gt_sdf, -0.3, 0.3).float()
#     inter_constraint = torch.where(gt_sdf == 0, torch.zeros_like(pred_sdf), torch.exp(- 1e2 * abs(pred_sdf_c)))
    inter_constraint = torch.where(gt_sdf == 0, torch.zeros_like(pred_sdf), abs(gt_sdf_c - pred_sdf_c))
#     print(sdf_constraint, inter_constraint)
# 
    gradient = diff_operators.gradient(pred_sdf, coords)
    
#     print(gradient.size, pred_sdf.size(), coords.size(), gt_normals.size())

    if kn != 0:
        norm = (1 - F.cosine_similarity(gradient, gt_normals, dim=-1))[..., None]
        normal_constraint = torch.where(gt_sdf == 0, norm ,torch.zeros_like(gradient[..., :1]))
    else :
        normal_constraint = torch.tensor([0]).float()
        
#     grad_constraint = torch.exp(-   torch.linalg.norm(gradient,dim=-1))
    grad_constraint =abs(1 - torch.clip(torch.linalg.norm(gradient,dim=-1), 0, 1))
    print("max min", torch.amax(torch.linalg.norm(gradient,dim=-1)) , torch.amin(torch.linalg.norm(gradient,dim=-1)) )
#     print(torch.where(gt_sdf == 0)[1])
#     print("g", gradient, abs(1 - torch.linalg.norm(gradient,dim=-1))[ torch.where(gt_sdf == 0)[1] ].sort()  )
    # else:
    #   normal_constraint = torch.tensor([0]).float()
    #   grad_constraint = torch.tensor([0]).float()
    # print(grad_constraint)

#     print(torch.abs(sdf_constraint).mean() * ks,inter_constraint.mean() * ki, normal_constraint.mean() * kn,grad_constraint.mean() * kg )
    # Exp      # Lapl
    # -----------------
    print(inter_constraint.mean(), normal_constraint.mean(),  grad_constraint.mean())
    return { 
            ...
            'grad_constraint': grad_constraint.mean() * kg}  # 1e1      # 5e1
...
#     inter_constraint = torch.where(gt_sdf == 0, torch.zeros_like(pred_sdf), 0.1- pred_sdf_c)
    inter_constraint = torch.where(gt_sdf == 0, torch.zeros_like(pred_sdf), abs(gt_sdf_c - pred_sdf_c))
    ...
    # Exp      # Lapl
    # -----------------
    return { 'latent_loss': kl * latent_loss(model_output),
