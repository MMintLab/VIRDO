...
class virdo_hypernet(nn.Module):
    ...
    def __init__(
        self, in_features, out_features, hyper_in_features=256, hl=2, **kwargs
    ):
        super().__init__()
        self.hl = hl
        ...
        self.hypo_net = modules.SingleBVPNet(
            out_features=out_features,
            type="relu",
            mode="hypo",
            in_features=in_features,
            hidden_features=256,
            num_hidden_layers=self.hl,
            outermost_linear=False,
            drop_out=False,
        )

        self.hyper_net = HyperNetwork(
            hyper_in_features=hyper_in_features,
            hyper_hidden_layers=0,
    ...
    def forward(self, model_input):
        if "model_out" in model_input.keys():
            input = {
                ...
                "model_out": model_input["model_out"],
            }
        else:
        ...
        return {
            "model_in": output["model_in"],
            "model_out": output["model_out"],
