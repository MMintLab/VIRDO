        ...
        **kwargs
        ...
        for i in range(num_hidden_layers):
            if i != self.latent_in - 1:
                self.net.append(
                    MetaSequential(BatchLinear(hidden_features, hidden_features), nl)
                )
            else:
                self.net.append(
                    MetaSequential(
...
class SingleBVPNet(MetaModule):
    ...
    def forward(self, model_input, params=None):
        ...
        for layer in range(self.num_hidden_layers + 2):
            if layer == 0:
                x = self.net.net[0][0](
                    input, params=get_subdict(params, "net"), layer_num=layer
                )
                try:
                ...
                x = self.net.net[layer][0](
                    x, params=get_subdict(params, "net"), layer_num=layer
                )
